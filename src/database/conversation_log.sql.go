...
const countConversationLogs = `-- name: CountConversationLogs :one
SELECT COUNT(*)
...
type CountConversationLogsParams struct {
	UserID       pgtype.UUID `json:"user_id"`
	ModelID      pgtype.UUID `json:"model_id"`
	ConnectionID pgtype.UUID `json:"connection_id"`
}

func (q *Queries) CountConversationLogs(ctx context.Context, arg CountConversationLogsParams) (int64, error) {
	row := q.db.QueryRow(ctx, countConversationLogs, arg.UserID, arg.ModelID, arg.ConnectionID)
	var count int64
	err := row.Scan(&count)
	return count, err
}

const createConversationLog = `-- name: CreateConversationLog :one
...
type CreateConversationLogRow struct {
...
func (q *Queries) CreateConversationLog(ctx context.Context, arg CreateConversationLogParams) (CreateConversationLogRow, error) {
	row := q.db.QueryRow(ctx, createConversationLog,
		arg.UserID,
		arg.ModelID,
		arg.RequestPayload,
		arg.ResponsePayload,
	...
	var i CreateConversationLogRow
	err := row.Scan(
		&i.ID,
		&i.UserID,
		&i.ModelID,
...
const listConversationLogs = `-- name: ListConversationLogs :many
...
type ListConversationLogsParams struct {
	UserID       pgtype.UUID `json:"user_id"`
	ModelID      pgtype.UUID `json:"model_id"`
	ConnectionID pgtype.UUID `json:"connection_id"`
...
type ListConversationLogsRow struct {
	ID               pgtype.UUID        `json:"id"`
	UserID           pgtype.UUID        `json:"user_id"`
	ModelID          pgtype.UUID        `json:"model_id"`
	RequestPayload   []byte             `json:"request_payload"`
...
func (q *Queries) ListConversationLogs(ctx context.Context, arg ListConversationLogsParams) ([]ListConversationLogsRow, error) {
	rows, err := q.db.Query(ctx, listConversationLogs,
		arg.UserID,
		arg.ModelID,
		arg.ConnectionID,
		arg.Offset,
	...
	defer rows.Close()
	var items []ListConversationLogsRow
	for rows.Next() {
		var i ListConversationLogsRow
		if err := rows.Scan(
			&i.ID,
			&i.UserID,
			&i.ModelID,
